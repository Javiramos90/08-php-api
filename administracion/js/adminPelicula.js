...
function validaPelicula(titulo, precio, id_director) {
    let errores = [];
    if (titulo.length <= 2 && titulo.length >= 50) {
        errores.push("El titulo debe contener entre 2 y 50 caracteres.");
    }
    if (!parseFloat(precio) || precio < 0) {
        errores.push("El precio tiene que ser un numero válido positivo.");
    }
        ...
        .then(peliculas => {
            ...
            peliculas.forEach(pelicula => {
                ...
                listaDirectores.forEach(director => {
                    ...
                    const sanitizedApellido = limpiarHTML(director.apellido);
                    optionsHTML += `
                    <option value="${director.id}" ${(director.id === directorSeleccionado.id) ? 'selected' : ''}> ${sanitizedNombre} ${sanitizedApellido}</option>
                `
                })
                ...
                tableBody.innerHTML += `
                    <tr data-id="${pelicula.id}">
                        ...
                        <td class="td-btn">
                            ...
                            <button class="edicion" onclick="updatePelicula(${pelicula.id})">Modificar</button>
...
function createPelicula(event) {
    ...
    let erroresValidaciones = validaPelicula(titulo, precio, id_director);
...
function updatePelicula(id) {
    ...
    let erroresValidaciones = validaPelicula(newTitulo, newPrecio, newIdDirector);
    if (erroresValidaciones.length > 0) {
        mostrarErrores(erroresValidaciones);
        return;
    }
